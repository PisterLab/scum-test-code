...
int main(void) {
    ...
    radio_setEndFrameRxCb(radio_rx_cb);
		...
		app_vars.IF_fine_clk_setting = scm3c_hw_interface_get_IF_fine();
				...
				rftimer_setCompareIn_by_id(rftimer_readCounter()+125000, TIMER_CB_TX_BEACON);
		#endif
...
void tx_beacon_callback(void) {
		...
		rftimer_setCompareIn_by_id(app_vars.current_count + 125000, TIMER_CB_TX_BEACON ); // send a packet in the dedicated slot
