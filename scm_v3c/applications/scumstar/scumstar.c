...
#define JOINED		2 // SENT A JOIN REQ. AND RECEIVED A VALID ACK WITH SCHEDULE INFO!

// === TIME VARIABLES === //
#define RX_EB_BACKOFF					1000
...
//#define RX_IF_BACKOFF								0

// === TIMER COMPARE REG. CALLBACKS === // USE OF 0 IS DEDICATED TO LOW-LEVEL RADIO DELAYS, USE AT YOUR OWN RISK
...
//#define SELECTED_SCUM						4 // PROG PORT 4, UART PORT LOGIC ANALYZER
//#define SELECTED_SCUM						77 // TEENSY OPTICAL PROG. PORT, NO UART
//#define SELECTED_SCUM						30
//#define SELECTED_SCUM						36
//#define SELECTED_SCUM						46
...
#define EB_ONLY										1
		...
		#define TX_LC_COARSE				24
		#define TX_LC_MID						29
		#define TX_LC_FINE					20
		
		// address
		#define MY_ADDRESS					0xAB30
...
#elif SELECTED_SCUM==36
		// LC CODES, CHANNEL 17
		#define RX_LC_COARSE				24
		#define RX_LC_MID						27
		#define RX_LC_FINE					21
		#define RX_LC_FINE_SYNC			RX_LC_FINE+RX_FINE_CODE_FUDGE
		#define TX_LC_COARSE				24
		#define TX_LC_MID						30
		#define TX_LC_FINE					24
		
		// address
		#define MY_ADDRESS					0xAB36
...
#elif SELECTED_SCUM==47
		// LC CODES, CHANNEL 17
		#define RX_LC_COARSE				25
		#define RX_LC_MID						19
		#define RX_LC_FINE					11
		#define RX_LC_FINE_SYNC			RX_LC_FINE+RX_FINE_CODE_FUDGE
		#define TX_LC_COARSE				25
		#define TX_LC_MID						26
		#define TX_LC_FINE					15
...
typedef struct {
		...
		uint8_t		rx_EBs_to_average; // only less than NUM_RX_EBS_FOR_AVG if it's at the beginning of a desync period
...
int main(void) {
    ...
    perform_calibration();
		...
		// initialize RADIO start and endframe callbacks TODO: make this a private function
		radio_setStartFrameRxCb(radio_startframe_cb);
    radio_setEndFrameRxCb(radio_rx_cb);
		radio_setEndFrameTxCb(tx_endframe_callback);
	
		// initialize RFTIMER compare register callbacks and interrupts TODO: make this a private function
		rftimer_init();
		...
		rftimer_set_callback_by_id(rx_startframe_timeout_callback, 	TIMER_CB_SF_TIMEOUT);
		...
		radio_rfOn();
		LC_FREQCHANGE(channel_vars.rx_coarse, channel_vars.rx_mid, channel_vars.rx_fine);
		...
		rftimer_setCompareIn_by_id(rftimer_readCounter()+6000, TIMER_CB_SF_TIMEOUT);
		...
		while(1) {
				...
				while(app_vars.rxpk_done == 0) {
						...
						if (app_vars.printflag == 1) {
							printf("fine: %d, my count: %d, guard time: %d, IF: %d \r\n", channel_vars.rx_fine_sync, time_sync_vars.rx_EB_timer, app_vars.current_count-time_sync_vars.rx_EB_start_reception_time, app_vars.IF_estimate);
...
void radio_rx_cb(uint32_t timestamp) {
    uint8_t i;
		...
		if (1) {
				...
				if ( (destination_address(app_vars.packet[DESTINATION_ADDRESS_MSB_INDEX], app_vars.packet[DESTINATION_ADDRESS_LSB_INDEX]) == PACKET_EB) && (app_vars.packet[SOURCE_ADDRESS_MSB_INDEX] == PREF_PARENT_MSB) && (app_vars.packet[SOURCE_ADDRESS_LSB_INDEX] == PREF_PARENT_LSB)) {
						...
						time_sync_vars.rx_EB_timer_from_packet = (((((temp_storage_1<<8)|(temp_storage_2))+1)*((uint32_t)(40000)))/32768)*100;
						...
						time_sync_vars.tx_join_req_timer_from_parent = (((((temp_storage_1<<8)|(temp_storage_2))+1)*((uint32_t)(5000)))/32768)*100;
						if (scumpong_vars.sync_state == DESYNCHED) {
								
								time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer - RX_EB_BACKOFF;
								// === for some reason, the timer gets messed up for very long EB periods, reasons unknown
								//rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer + RX_EB_BACKOFF, TIMER_CB_RX_EB );
								//if (SELECTED_SCUM==4) {
						...
						else if ((scumpong_vars.sync_state==SYNCHED)||(scumpong_vars.sync_state==JOINED)) {
								
							// update the rx EB timer :D
							// over-the-air "PLL" goes here.
						
							if (((app_vars.current_count-time_sync_vars.rx_EB_start_reception_time) > (RX_EB_GUARD_TIME_TARGET+RX_EB_PACKET_DURATION)+500)) {
									if (app_vars.dont_pll == 0) {
											time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer + 200;
									}
									else {
							...
							else if (((app_vars.current_count-time_sync_vars.rx_EB_start_reception_time) < (RX_EB_GUARD_TIME_TARGET+RX_EB_PACKET_DURATION)-200)) {
									if (app_vars.dont_pll == 0) {
											time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer - 200;
									}
									else {
									...
									if (app_vars.dont_pll == 0) {
											time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer + 50;
									}
									else {
											time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer;
									}
							...
							else if ((app_vars.current_count-time_sync_vars.rx_EB_start_reception_time) < (RX_EB_GUARD_TIME_TARGET+RX_EB_PACKET_DURATION)) {
									//time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer - 50;
									time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer;
									app_vars.dont_pll = 1;
									//tune_fine_rx_sync_code(app_vars.IF_estimate);
								...
								//rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer , TIMER_CB_RX_EB );
								//time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer - RX_EB_BACKOFF;
								//rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer - RX_IF_BACKOFF - RX_EB_BACKOFF, TIMER_CB_RX_EB );
										
								time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer;
								rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer - RX_IF_BACKOFF - RX_EB_BACKOFF, TIMER_CB_RX_EB );

								app_vars.IF_on_early = 1;
								time_sync_vars.i_was_early_by = app_vars.current_count-time_sync_vars.rx_EB_start_reception_time;
								//rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer - RX_EB_BACKOFF, TIMER_CB_RX_EB );
								//rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer + RX_EB_BACKOFF, TIMER_CB_RX_EB );
						}
						...
						scumpong_vars.parent_address = (temp_storage_1<<8)|(temp_storage_2);
						if (scumpong_vars.sync_state == DESYNCHED) {
								//if (((app_vars.current_count - app_vars.previous_count) > time_sync_vars.rx_EB_timer_from_packet*1.5)) {
								if (((app_vars.current_count - app_vars.previous_count) > TIME_INTERVAL*1.5)) {
										// missed an EB during sync procedure.
										if (time_sync_vars.rx_EB_timer == 0) {
												//time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer_from_packet - 2000;
												time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer_from_packet; 
										}
								}
								else {
										// packet rate seems correct
										// use time rate
										time_sync_vars.rx_EB_timer = app_vars.current_count - app_vars.previous_count - RX_EB_BACKOFF; // maintain a "count"
								}
						}
				}
				else if (destination_address(app_vars.packet[DESTINATION_ADDRESS_MSB_INDEX], app_vars.packet[DESTINATION_ADDRESS_LSB_INDEX]) == PACKET_FOR_ME) { // indicates a packet unicast to me
						if ((scumpong_vars.sync_state == SYNCHED)||(scumpong_vars.sync_state == JOINED)) { // 
								gpio_15_clr();
								gpio_14_clr();
								gpio_13_clr();
								gpio_12_clr();
						}
						if (((app_vars.packet[4]==0x00)&&(app_vars.packet[5]==0x44))) { // indicates either join request ack. (from parent) or join request (from new child)
							
							if (((app_vars.packet[SOURCE_ADDRESS_MSB_INDEX] == PREF_PARENT_MSB)&&(app_vars.packet[SOURCE_ADDRESS_LSB_INDEX]==PREF_PARENT_LSB))) { // parent responded to my join request
										temp_storage_1 = ((uint32_t)(app_vars.packet[6]));
										temp_storage_2 = ((uint32_t)(app_vars.packet[7]));
										time_sync_vars.tx_dedicated_slot_from_parent = (((((temp_storage_1<<8)|(temp_storage_2))+1)*((uint32_t)(5000)))/32768)*100; // extract dedicated slot TX time
										temp_storage_1 = ((uint32_t)(app_vars.packet[9]));
										temp_storage_2 = ((uint32_t)(app_vars.packet[10]));
										time_sync_vars.tx_EB_timer_from_parent = (((((temp_storage_1<<8)|(temp_storage_2))+1)*((uint32_t)(40000)))/32768)*100; // extract dedicated EB TX time
										scumpong_vars.sync_state = JOINED;
										//printf("tx EB at: %d\r\n", time_sync_vars.tx_EB_timer_from_parent);
										radio_rfOff(); // turn radio off immediately
										gpio_13_clr();
										gpio_12_clr();
										printf("JOINED, tx EB at: %d, ded. slot at %d \r\n", time_sync_vars.tx_EB_timer_from_parent, time_sync_vars.tx_dedicated_slot_from_parent);
								}
								else { // set everything up so that the child knows when to speak:
										// store child address:

										scumpong_vars.child_address_MSB = app_vars.packet[SOURCE_ADDRESS_MSB_INDEX];
										scumpong_vars.child_address_LSB = app_vars.packet[SOURCE_ADDRESS_LSB_INDEX];
				...
				if (app_vars.sync_counter == NUM_RX_EBS_FOR_SYNC) {
						...
						app_vars.sync_counter++; // disable so that more advanced states are not treated as SYNCHED again
				}
				
				// response state machine
				if (scumpong_vars.sync_state == SYNCHED) { // if synched - send a join request
						radio_rfOff(); // do not listen forever
						gpio_13_clr();
						gpio_12_clr();
						if ( (ONLY_LISTEN == 0)&&(EB_ONLY==0)) { // debug mode - do not send join requests
								rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_join_req_timer_from_parent, TIMER_CB_TX_UPLINK ); // send a join request
						}
						if ( EB_ONLY == 1) { // debug mode - only send EBs 
								scumpong_vars.sync_state = JOINED;
								time_sync_vars.tx_EB_timer_eb_only = time_sync_vars.rx_EB_timer - EB_TIMER_SKEW;
								time_sync_vars.tx_EB_timer_from_parent = time_sync_vars.rx_EB_timer;
						}
						// disable the weird hack interrupt
						rftimer_disable_interrupts_by_id(	TIMER_CB_SF_TIMEOUT		);
				}
				else if (scumpong_vars.sync_state == JOINED) { // if join request was acknowledged, I am in the network, send in my dedicated slot and wait for response
						radio_rfOff(); // do not listen forever
						gpio_13_clr();
						gpio_12_clr();
						scumpong_vars.desync_risk = 0;
						if (scumpong_vars.ack_join_request == 1) {
								rftimer_setCompareIn_by_id(rftimer_readCounter() + 500, TIMER_CB_TX_UPLINK ); // acknowledge the child join request.
						}
						else {
								if (EB_ONLY == 0) { // only send in dedicated slot if we are not in EB only debug mode
										rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_dedicated_slot_from_parent, TIMER_CB_TX_UPLINK ); // send a packet in the dedicated slot
								}
						}
						//rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_EB_timer_from_parent - RX_IF_BACKOFF, TIMER_CB_TX_EB ); // send an EB
						//if (EB_ONLY == 1) {
						//		time_sync_vars.tx_EB_timer_from_parent = time_sync_vars.rx_EB_timer - EB_TIMER_SKEW;
						//}
						if (EB_ONLY == 1) {
								rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_EB_timer_eb_only - RX_IF_BACKOFF, TIMER_CB_TX_EB ); // send an EB
						}
						else {
								rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_EB_timer_from_parent, TIMER_CB_TX_EB ); // send an EB
						}
				}
				...
				/*if (scumpong_vars.sync_state == DESYNCHED) {
						printf("my count: %d, RX EB packet: %d, join req. time %d, IF: %d \r\n", app_vars.current_count - app_vars.previous_count, time_sync_vars.rx_EB_timer_from_packet, time_sync_vars.tx_join_req_timer_from_parent, app_vars.IF_estimate);
				}
				if ((scumpong_vars.sync_state == SYNCHED)||(scumpong_vars.sync_state == JOINED)) {
						printf("my count: %d, will listen at: %d, IF: %d \r\n", app_vars.current_count - app_vars.previous_count, time_sync_vars.rx_EB_timer, app_vars.IF_estimate);
						printf("time between rf on and rx end: %d\r\n", app_vars.current_count-time_sync_vars.rx_EB_start_reception_time);
				}*/
				
				// debug prints:
				app_vars.previous_count = app_vars.current_count;
				
				app_vars.source_address = (app_vars.packet[0]<<8) + app_vars.packet[1];
				app_vars.destin_address = (app_vars.packet[2]<<8) + app_vars.packet[3];
        ...
        if(app_vars.ADC_counter < (5685-5)) {
            app_vars.IF_fine_clk_setting -= 1;
				...
				set_IF_clock_frequency(app_vars.IF_coarse_clk_setting, app_vars.IF_fine_clk_setting, 0);
...
void test_rf_timer_callback(void) {
		...
		if (app_vars.IF_on_early == 1) {
				rftimer_setCompareIn_by_id(rftimer_readCounter() + RX_IF_BACKOFF, TIMER_CB_RX_EB);
				...
				if ((scumpong_vars.sync_state == SYNCHED)||(scumpong_vars.sync_state == JOINED)) {
						LC_FREQCHANGE(channel_vars.rx_coarse, channel_vars.rx_mid, channel_vars.rx_fine_sync);
						//LC_FREQCHANGE(channel_vars.rx_coarse, channel_vars.rx_mid, channel_vars.rx_fine);
					
						// start a timeout timer
						rftimer_setCompareIn_by_id(rftimer_readCounter() + RX_TIMEOUT_TIME, TIMER_CB_RX_TIMEOUT); // 5000 = 10ms
...
void join_request_timer_callback(void) {
		...
		if (scumpong_vars.ack_join_request == 1) {
				...
				app_vars.tx_packet[7] = 0xF9; // give child its dedicated slot (hard coded for now...)
				app_vars.tx_packet[8] = 0x15;
				packet_timer_32k = (time_sync_vars.tx_EB_timer_from_parent/4000000)*32768;
				...
				// indicate that I DO want to listen for an ACK for this downlink packet:
				...
				app_vars.tx_packet[3] = (uint8_t)((scumpong_vars.parent_address)&0xFF); // destination address
				app_vars.tx_packet[4] = 0x00;
...
void transmit_EB_timer_callback(void) {
		...
		if (EB_ONLY == 0) {
				...
				rftimer_setCompareIn_by_id(time_sync_vars.current_downlink_time + (((((0x66<<8)|(0x66))+1)*((uint32_t)(5000)))/32768)*100, TIMER_CB_TX_RX_DELAY);
		}
...
void receive_delay_callback(void) {
		...
		radio_rxEnable();
		radio_rxNow();
		rftimer_setCompareIn_by_id(rftimer_readCounter() + 20000, TIMER_CB_RX_TIMEOUT);
		gpio_12_set();
...
void rx_timeout_callback(void) {
		...
		else if ((scumpong_vars.last_EB_received == 0)&&(scumpong_vars.in_case_of_EB_miss == 0)) { // missed an EB, have not yet listened for join req.
				app_vars.current_count = rftimer_readCounter();
				//if (scumpong_vars.desync_risk == 0) {
						//rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer - RX_EB_BACKOFF - RX_TIMEOUT_TIME - RX_IF_BACKOFF - RX_EB_GUARD_TIME_TARGET, TIMER_CB_RX_EB);
						rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer - RX_EB_BACKOFF - RX_IF_BACKOFF - RX_EB_GUARD_TIME_TARGET-2500, TIMER_CB_RX_EB);
						app_vars.IF_on_early = 1;
						if (EB_ONLY == 0) {
								rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_EB_timer_from_parent - RX_EB_BACKOFF - RX_TIMEOUT_TIME + time_sync_vars.i_was_early_by + RX_EB_PACKET_DURATION, TIMER_CB_TX_EB ); // still send an EB
						}
						else {
								//rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_EB_timer_eb_only - RX_EB_BACKOFF - RX_TIMEOUT_TIME - RX_EB_GUARD_TIME_TARGET - RX_EB_PACKET_DURATION, TIMER_CB_TX_EB ); // still send an EB
								rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_EB_timer_eb_only - RX_EB_BACKOFF - RX_TIMEOUT_TIME - RX_EB_PACKET_DURATION - 1000, TIMER_CB_TX_EB ); // still send an EB
						}
						time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer - RX_EB_BACKOFF - 2500;
				//}
				//else if (scumpong_vars.desync_risk == 1) {
				//		rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.rx_EB_timer, TIMER_CB_RX_EB);
				//		rftimer_setCompareIn_by_id(app_vars.current_count + time_sync_vars.tx_EB_timer_from_parent, TIMER_CB_TX_EB ); // still send an EB
				//		time_sync_vars.rx_EB_timer = time_sync_vars.rx_EB_timer;
				//}
...
void tx_endframe_callback(uint32_t timestamp) {
		...
		// in case of ack - delay for a little bit between TX and RX, otherwise the radio is unhappy
		if (scumpong_vars.listen_for_ack == YES_LISTEN_FOR_ACK) {
				rftimer_setCompareIn_by_id(rftimer_readCounter() + 500, TIMER_CB_TX_RX_DELAY);
		}
		memset(app_vars.packet, 0, sizeof(app_vars.packet));
...
void rx_startframe_timeout_callback(void) {
		...
		rftimer_setCompareIn_by_id(rftimer_readCounter()+100000, TIMER_CB_SF_TIMEOUT);
}
...
uint8_t destination_address(uint8_t msb, uint8_t lsb) {
		...
		else if ((msb == (uint8_t)(MY_ADDRESS>>8)) && (lsb == (uint8_t)(MY_ADDRESS&0xFF))) {
...
uint8_t packet_type(uint8_t msb, uint8_t lsb) {
		uint8_t type;
}
