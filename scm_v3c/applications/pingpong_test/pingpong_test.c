...
void cb_endFrame_rx(uint32_t timestamp) {
    ...
    // In this demo code, it is assumed the OpenMote is sending packets with 20B
    // payloads
    if (app_vars.packet_len != LEN_RX_PKT) {
        ...
        if (radio_getCrcOk() == false) {
            ...
            // If this is the first valid packet received, start timers for next
            // expected packet
            if (app_vars.doing_initial_packet_search == true) {
                // Clear this flag to note we are no longer doing continuous RX
                // listen
            ...
            // Do this later in the ISR to make sure this register has settled
            // before trying to read it (the register is on the adc clock
            // domain)
            app_vars.cdr_tau_value = radio_get_cdr_tau_value();

            // Prepare ack - for this demo code the contents are arbitrary
            // The OpenMote receiver is looking for 30B packets - still on
            // channel 11 Data is stored in app_vars.packet[]
