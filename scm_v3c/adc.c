...
void adc_enable_interrupts(void) {
	ISER |= (0x1 << 3);
	printf("ADC enable interrupts: ISER: %x\n", ISER);
}
...
void gpio_onchip_config_adc(unsigned int gpi_control, unsigned int gpo_read) {
	...
	*/
	/*
	// Commented out by DL 9/15/22 
	// get_GPI_control not present in current scm3c_hw_interface
	// may need to be rewritten, but low priority at this time.
