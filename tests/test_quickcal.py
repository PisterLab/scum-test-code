...
def test_compilation():
    ...
    result = syscall("%KEIL_UV_DIR%\\UV4.exe -b scm_v3c\\applications\\quick_cal\\quick_cal.uvprojx -o \"output.txt\"")
    assert result <= 1
    
def test_bootload():
    syscall("echo bootload SCuM...")
    
    result = syscall("python scm_v3c\\bootload\\bootload.py -tp %PORT_TEENSY% -i scm_v3c\\applications\\quick_cal\\Objects\\quick_cal.bin")
    assert result == 0
    
    syscall("echo bootload OpenMote...")
    
    result = syscall("python scm_v3c\\bootload\\cc2538-bsl.py  -e --bootloader-invert-lines -w -b 400000 -p %PORT_OPENMOTE% scm_v3c\\applications\\quick_cal\\openmote-b-24ghz.ihex")
    assert result == 0
